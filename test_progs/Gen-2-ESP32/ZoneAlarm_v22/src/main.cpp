 ...
 * This is v2.2 of the Zone Alarm Project.
 * This version runs on 2 CPU Cores using some FreeRTOS functions
 * and adds alarm trigger identification to the LCD screen
*/
...
void Task1code(void *pvParameters)                          // CPU Core 1 controls Alarm Functions
{
    for(;;)
    {
        ...
        if (myKeyPress == '*')                              // can be relocked or clear PW entry anytime with a '*' press
        {
        ...
        if (armed)
        {
            ...
            if (position == 4)                     	        // if 4 digits were entered correctly
            {
                ...
                position = 0;                               // reset PW counter back to ZERO
            }
        }
    }
}
...
void setLocked(int locked)                                  // function declaration for setLocked function
{
    if (!locked)                                            // if door is locked / closed
    {
        ...
        FastLED.show();
        servoObj.write(180);                                // unlock servo: move to position OPEN = 180
    }
    else // if (locked)
    {
...
void printTime()                                            // prints Time to the Serial Terminal (for debug)
{ 
    Serial.print(String(rtc.hour()) + ":");                 // Print Hour: format: 12:34:56 Su MM/DD/YY
    if (rtc.minute() < 10)
    ...
    Serial.print(" - ");
    Serial.print(String(rtc.month()) + "/" + String(rtc.date()) + "/"); // Print date
...
void printTimeScrn()
{
    ...
    lcd.print("/");
    lcd.print(rtc.year());                                  // BUGFIX: DO NOT USE "println" (artifacts shown))
}

void I2C_20x4LCD_set()                                      // PowerUp Splash screen
{
...
void setup()
{
    ...
    xTaskCreatePinnedToCore(                                // instantiate Task to run `Task0Code` function on CPU 0
    ...
    xTaskCreatePinnedToCore(                                // instantiate Task to run `Task1Code` function on CPU 1
...
void loop() {}    // Not needed since we are using FreeRTOS Tasks